...
public class ArticleRepositoryIT {
  ...
  @Test
  void save() {
    Article aritcle = new Article("title", new ArticleContent("content", "type"));
    articleRepository.save(aritcle);

    SqlRowSet rsArticle =
        jdbcTemplate.queryForRowSet("select * from article where id = ?", aritcle.getId());
    assertThat(rsArticle.next()).isTrue();
    assertThat(rsArticle.getString("title")).isEqualTo("title");

    SqlRowSet rsContent =
        jdbcTemplate.queryForRowSet("select * from article_content where id = ?", aritcle.getId());
